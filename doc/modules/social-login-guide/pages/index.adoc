:sample-project: jmix-social-login-sample
...
. You can follow the guide step-by-step, or get the completed sample project to explore the finished implementation and experiment with the functionality.

* Either https://github.com/jmix-framework/{sample-project}/archive/refs/heads/{page-origin-branch}.zip[download^] and unzip the source repository
...
The `configureJmixSpecifics(HttpSecurity http)` method adds application-specific settings on top of the standard Jmix security configuration. It defines the OAuth2 login process by setting the login page, handling user data from the authentication provider, and managing the post-login behavior:
...
The `oauth2UserService()` method handles GitHub users:
